...
describe('TaskService', () => {
  ...
  let taskRepository: Partial<AbstractTaskRepository>;
  let categoryRepository: Partial<Repository<CategoryEntity>>;

  beforeEach(() => {
    taskRepository = {
      initialize: jest.fn(() => new TaskEntity()),
      save: jest.fn(async (task: TaskEntity) => ({ ...task } as TaskEntity)),
      findPaginated: jest.fn(),
      findDetailedById: jest.fn(),
      findActiveWithCategory: jest.fn(),
      softDeleteById: jest.fn(),
      mapToInterface: jest.fn(),
    };
